...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # No. of input dimensions = (C*H*W)
    # No. of output dimensions = (F*H'*W')

    w_mtx = np.zeros((F, H_out, W_out, C, H, W))
    for f in range(F):
        for i in range(H_out):
            for j in range(W_out):
                cur_h = i * stride
                cur_w = j * stride
                w_mtx[f, i, j, :, cur_h:cur_h+HH, cur_w:cur_w+WW] = w[f]

    w_mtx = w_mtx.reshape((F * H_out * W_out, -1))
    x_mtx = x.reshape(N, -1)

    out = x_mtx.dot(w_mtx.T).reshape(N, F, H_out, W_out)
    for f in range(F):
        out[:, f, :, :] += b[f]
